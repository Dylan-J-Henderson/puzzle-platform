...
const useRubiksCube = () => {
  ...
  const [cubies, setCubies] = useState(() => {
    const savedState = getPuzzleState('rubiks-cube');
    return savedState || initializeCubies();
  });

  useEffect(() => {
    if (animationPhase === 'scrambling') {
      const scramble = generateScramble(SIZE, 20);
  ...
  useEffect(() => {
    savePuzzleState('rubiks-cube', cubies);
  ...
  const applyScramble = useCallback(async (scramble) => {
    for (const move of scramble) {
      await rotateLayer(move.axis, move.layer, move.direction, 30);
    }
  }, [rotateLayer]);
