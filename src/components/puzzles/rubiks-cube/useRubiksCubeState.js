...
export const useRubiksCubeState = () => {
    ...
    for (let x = -1; x <= 1; x++) {
      for (let y = -1; y <= 1; y++) {
        for (let z = -1; z <= 1; z++) {
          ...
          const materials = [
            new THREE.MeshLambertMaterial({ color: x === 1 ? colors[0] : 0x000000 }),
            new THREE.MeshLambertMaterial({ color: x === -1 ? colors[1] : 0x000000 }),
            new THREE.MeshLambertMaterial({ color: y === 1 ? colors[2] : 0x000000 }),
            new THREE.MeshLambertMaterial({ color: y === -1 ? colors[3] : 0x000000 }),
            new THREE.MeshLambertMaterial({ color: z === 1 ? colors[4] : 0x000000 }),
            new THREE.MeshLambertMaterial({ color: z === -1 ? colors[5] : 0x000000 }),
          ];
  ...
  const checkIfSolved = () => {
    ...
    const solved = cubiesRef.current.every((cubie) => {
      ...
      const posMatch = 
        Math.abs(currentPos.x - (initialPos.x * 1)) < threshold &&
        Math.abs(currentPos.y - (initialPos.y * 1)) < threshold &&
        Math.abs(currentPos.z - (initialPos.z * 1)) < threshold;
    ...
    const animateShrink = () => {
      ...
      if (progress < 1) {
        ...
        scrambleSequence.forEach((move) => {
          const moveHandlers = {
            'R': () => instantRotate('x', 1, 1),
            "R'": () => instantRotate('x', 1, -1),
            'L': () => instantRotate('x', -1, -1),
            "L'": () => instantRotate('x', -1, 1),
            'U': () => instantRotate('y', 1, 1),
            "U'": () => instantRotate('y', 1, -1),
            'D': () => instantRotate('y', -1, -1),
            "D'": () => instantRotate('y', -1, 1),
            'F': () => instantRotate('z', 1, 1),
            "F'": () => instantRotate('z', 1, -1),
            'B': () => instantRotate('z', -1, -1),
            "B'": () => instantRotate('z', -1, 1),
          };
    ...
    const animateShrink = () => {
      ...
      if (progress < 1) {
        ...
        cubiesRef.current.forEach((cubie) => {
          cubie.position.set(
            cubie.userData.initialPosition.x * 1,
            cubie.userData.initialPosition.y * 1,
            cubie.userData.initialPosition.z * 1
          );
