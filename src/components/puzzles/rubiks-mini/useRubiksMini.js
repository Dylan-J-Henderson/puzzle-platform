...
const useRubiksMini = () => {
  ...
  const [cubies, setCubies] = useState(() => {
    const savedState = getPuzzleState('rubiks-mini');
    return savedState || initializeCubies();
  });

  useEffect(() => {
    if (animationPhase === 'scrambling') {
      const scramble = generateScramble(SIZE, 15);
  ...
  useEffect(() => {
    savePuzzleState('rubiks-mini', cubies);
    ...
    return new Promise((resolve) => {
      ...
      const animate = () => {
        ...
        if (progress < 1) {
          ...
          // Apply final rotation to cubies
  ...
  const applyScramble = useCallback(async (scramble) => {
    for (const move of scramble) {
      await rotateLayer(move.axis, move.layer, move.direction, 50);
    }
  }, [rotateLayer]);
  ...
  const handleFaceClick = useCallback((intersect, dragDelta) => {
    ...
    // Determine which axis and layer to rotate based on drag direction and face
    ...
    // Determine layer based on click position
    const half = (SIZE - 1) / 2;
